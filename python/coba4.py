...
from SharepointService import Sharepoint
import requests
import ast
import json
...
# POST Login
@app.route('/api/sharepoint/login', methods=['POST'])
def Login():
  content = request.get_json()
  getUsername = content['login']['username']
  getPassword = content['login']['password']
...
# POST
@app.route('/api/sharepoint/list', methods=['POST'])
def ListData():
  content = request.get_json()
  ProccessToSharepoint = Sharepoint(content)

  forReturn = ProccessToSharepoint.Proccess(Session)
  return json.dumps(forReturn)
#
# # POST article Data
